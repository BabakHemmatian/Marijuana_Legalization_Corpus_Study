...
class ModelEstimator(object):
    ...
    def Create_New_Sets(self, indices):
        ...
        if isinstance(self, NNModel): # for NN
            ...
            assert (len(self.sets['dev']) - len(self.sets['test'])) <= 1, "The development and test set sizes are not equal"
            assert len(self.sets['dev']) + len(self.sets['test']) + len(self.sets['train']) == len(indices), "The sizes of the training, development and test sets do not add up to the number of posts on file"
            ...
            assert len(self.LDA_sets['train']) + len(self.LDA_sets['eval']) == len(indices), "The training and evaluation set sizes do not correspond to the number of posts on file"
        ...
        if (isinstance(self, NNModel) and
            ...
            if (isinstance(self, NNModel) and
                ...
                if isinstance(self, NNModel): # for NN
                    ...
                    # ensure set sizes are correct
                    assert len(self.sets['dev']) - len(self.sets['test']) < 1, "The sizes of the development and test sets do not match"
                    l=list(indices[-1]) if (self.all_ and not self.special_doi) else len(list(indices))
                    assert len(self.sets['dev']) + len(self.sets['test']) + len(self.sets['train']) == l, "The sizes of the training, development and test sets do not add up to the number of posts on file"
...
class LDAModel(ModelEstimator):
    ...
    def Get_Indexed_Dataset(self):
        with open(self.fns["lda_prep"],'r') as f:
            ...
            if not self.all_:
                assert Path(self.fns["random_indices"]).is_file(), "Could not find the set of chosen random indices on file"
    ...
    def Topic_Asgmt_Retriever_Multi(self, indexed_comment):
        ...
        for word_id, freq in bow: # iterate over the word-topic assignments
            try:
                ...
                assert all([ word_id not in phis[i] for i in range(self.num_topics) ]), "Word-topic probability assignment error"
                continue
    ...
    def Get_Topic_Contribution(self):
        ...
        if not Path(self.fns["topic_cont"]).is_file(): # if not
            ...
            Q = input('Topic contribution estimations were found on file. Do you wish to delete them and calculate contributions again? [Y/N]')
    ...
    def Get_Top_Topic_Theta(self):
        ...
        if not Path(self.fns["theta"]).is_file(): # if not
            ...
            Q = input('Theta estimations were found on file. Do you wish to delete them and calculate probabilities again? [Y/N]')
                                    ...
                                    if body.strip() != "":
                                        ...
                                        for topic,indices in sampled_indices.items():
                                            if counter-1 in indices:
                                                ...
                                                writer_keys.writerow([counter-1,sampled_ids[counter-1],month,self.dates[0][1]+year_counter,topic,sampled_probs[topic][itemindex]])
                ...
                for comm_index,comment in enumerate(fin): # iterate over the original comments
                    ...
                    for topic,indices in sampled_indices.items():
                        if comm_index in indices:
                            ...
                            # print the values to CSV file
                            itemindex = sampled_indices[topic].index(comm_index)
                            writer_keys.writerow([comm_index,sampled_ids[comm_index],month,self.dates[0][1]+year_counter,topic,sampled_probs[topic][itemindex]])
