...
state_index = 2
county_index = 3
author_data = {} # stores comment count, state, county for each author

for root, dirs, files in os.walk("Marijuana_Leg_Analysis/location"):
    for file in files:
        if file.endswith(".csv"):
            ...
            for i in range(len(file_data)):
                if i > 0:
                    ...
                    # get location, look up demographic, find comment length
                    author = row[0].rstrip()
                    count = row[3]
...
for j in range(len(pol_data)):
    if j > 0:
        ...
        if state not in poll_data_dict:
            poll_data_dict[state] = {pol_row[10]: per_GOP}
        else:
            poll_data_dict[state][pol_row[10]] = per_GOP
...
num_words_data = {}
num_words_count = 0
with open("Marijuana_Leg_Analysis/LDA_full-corpus_True_50/data_for_R-f.csv", newline='') as csvfile:
    ...
    for i, comment in enumerate(query_results):
        ...
        if (current_author != "[deleted]" and curr_perc is not None):
            ...
            if current_author in author_data and author_data[current_author][count_index] >= 10 and author_data[current_author][country_index] == 'US':
                ...
                party_data = 0
                # Poll data
                if state in poll_data_dict and county in poll_data_dict[state]:
                    party_data = poll_data_dict[state][county]
                # populate the lists
